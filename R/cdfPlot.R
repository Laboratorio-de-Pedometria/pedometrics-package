...
cdfPlot <- 
  function (obj, ind, units.cdf = "percent", type.plot = "s", 
    ...
    if (!requireNamespace("spsurvey")) stop("spsurvey package is missing")
    ...
    if (type.cdf == "continuous") {
      ty <- c("l", "s")
      if (any(ty == type.plot)) {
        graphics::plot(cdfdata[, 1], cdfdata[, 2], type = type.plot, ylim = ylimit, xlab = xlbl,
          ylab = ylbl, log = logx, ...)
      ...
      if (len > 1) {
        ...
        tmp <- cbind(matrix(c(value, value[length(value)]), ncol = 2, byrow = TRUE), rep(NA, len))
    ...
    if (show.param) {
      ...
      graphics::legend(x = legx, y = legy, xjust = xjust, yjust = yjust,
        legend = c("CDF Estimate", paste(conflev, "% CL", sep = "")), lty = c(1, 3),
        lwd = c(1, 1.5), bty = "n", cex = 1)
    }
    if (!is.null(ylbl.r)) {
      ...
      if (ylbl.r == "Same") {
        ...
        graphics::axis(side = 4, at = yl.lab, labels = as.character(round(yr.lab)))
