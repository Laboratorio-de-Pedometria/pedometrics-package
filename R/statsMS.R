...
#' [pedometrics::buildMS()]. The main objective is to compare several linear models using several
#' performance statistics. Such statistics can then be used to rank the linear models and identify,
#' for example, the best performing model, given the selected performance statistics.
#' 
#' An important feature of [pedometrics::buildMS()] is that it uses the information about the
#' initial number of candidate predictor variables offered to the build the model to calculate
#' penalized or adjusted measures of model performance. Such information is recorded as an attribute
#' of the final model selected by [pedometrics::buildMS()]. This feature was included in
#' [pedometrics::statsMS()] because data-driven variable selection results biased linear models (too
#' optimistic), and the effective number of degrees of freedom is close to the number of candidate
#' predictor variables initially offered to the model (Harrell, 2001).
#' 
#' @return
